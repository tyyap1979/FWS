...
//		logger.debug("_req.getScheme() = "+_req.getScheme());
		...
		String[] adminContext = ThemesServices.getInstance ().getAdminContext(domainName, servletArray[servletArray.length-2]);
				
		logger.debug("adminContext[1] = "+adminContext[1]);
		logger.debug("servletArray[servletArray.length-2] = "+servletArray[servletArray.length-2]);
		
		if(adminContext[1]!=null && adminContext[1].equals (servletArray[servletArray.length-2])){
			moduleName = "admin";			
		}else{
			moduleName = "website";
		}
		...
		_shopInfoBean.setShopContext(servletArray[servletArray.length-2]);
		_clientBean.setRequestFilename (pageName);
		
		logger.info(adminContext[0]+(moduleName==null?"":"/"+moduleName)+"/"+pageName+"["+_clientBean.getLanguage()+"]");
	}
