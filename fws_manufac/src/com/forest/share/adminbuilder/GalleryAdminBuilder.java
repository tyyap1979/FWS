...
public class GalleryAdminBuilder extends GenericAdminBuilder {
	private Logger logger = Logger.getLogger (GalleryAdminBuilder.class);	
	
	public GalleryAdminBuilder(ShopInfoBean mShopInfoBean, ClientBean mClientBean, Connection conn, 
	...
	public JSONObject requestJsonHandler()throws Exception{
		...
		if(GeneralConst.CREATE.equals (_reqAction)){							
			json = requestCreate();
			UploadAdminBuilder.moveTempPhoto(_attrReqDataDetailMap[0], _shopInfoBean, "gallery/"+(String)_attrReqDataMap[0].get(GalleryDef.gid.name));			
			...
			for(int i=0; i<count; i++){
				UploadAdminBuilder.removeFolder(_shopInfoBean, "gallery/"+_deleteArrCode[i]);
			}		
		}else{
	...
	protected JSONObject requestCreate()throws Exception{
		...
		for(int i=0; i<_attrReqDataDetailMap[0].length; i++){					
			if(_attrReqDataDetailMap[0][i]!=null){
				filename = (String)_attrReqDataDetailMap[0][i].get(GalleryDetailDef.filename.name);
				filename = filename.substring(filename.lastIndexOf("/")+1);
				_attrReqDataDetailMap[0][i].put(GalleryDetailDef.filename.name, filename);				
			}
		}
		...
		if(subClass!=null){				
			...
			for(int i=0; i<subClass.length; i++){				
				...
				for(int x=0; x<dataList.size (); x++){				
					...
					rowData.put(GalleryDetailDef.filename.name, "/photo/gallery/"+(String) rowData.get(GalleryDetailDef.gid.name)+"/"+(String) rowData.get(GalleryDetailDef.filename.name));
				}
